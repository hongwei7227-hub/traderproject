...
└─────────────┘                └────────┬─────────┘
                        ...
                        ┌───────────────┼───────────────┐
                        ▼               ▼               ▼
                   LLM 供应商      PostgreSQL      Java 微服务
                   （4 种协议）                   （下单/分析师/支付）
```

**在本仓库内**：Python 平台（`server/`）、React 前端（`web/`）。

**在本仓库外**：下单执行、分析师数据、支付这三个 Java 微服务是独立工程，平台通过 HTTP 调用它们。
...
| 项 | 状态 |
|---|---|
| 后端测试 | **559 通过**，内存 SQLite，不需要任何外部服务 |
| 前端测试 | **117 通过** |
...
租户隔离 · 模型目录与选型链 · 配额两阶段 · 熔断与降级 · 声明式管线 · 流式协议与重放 · 持久化与迁移 · 身份校验与中间件 · 四种线协议 · 工具注册表与沙箱 · 组装根 · 前端对话/会话列表/设置页
...
| Redis 缓存适配器 | 配置项已有，`adapters/cache/` 未建。**键构造器必须强制前置租户段**——HTTP 层漏判时，缓存层要能自然落空，构成纵深防御 |
